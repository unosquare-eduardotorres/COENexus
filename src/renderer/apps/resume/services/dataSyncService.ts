...
export const dataSyncService = {
  ...
  async fetchRecords(source: SyncSourceType): Promise<SyncRecord[]> {
    ...
    return (raw as any[]).map((r: any) => ({ ...r, pipelineStatus: r.status, failed: r.failed === 1 || r.failed === true }));
  },
  ...
  retryNotProcessed(
    ...
    return new Promise((resolve, reject) => {
      ...
      const cleanup = window.api.sync.onProgress((data: any) => {
        if (data.type === 'record') {
          const record: SyncRecord = { ...data.record, pipelineStatus: data.record.status, failed: data.record.failed ?? false };
