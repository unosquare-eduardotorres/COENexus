/**
 * Shared SVG icon components for the command-center app.
 *
 * Centralised here to eliminate duplication across report pages and drawers.
 * Each icon accepts an optional `size` override (defaults to 16).
 */
...
export function SortableColumnIcon({ active, dir }: { active: boolean; dir: 'asc' | 'desc' }) {
  if (!active) {
    return (
...
export function SmallFilterIcon({ active, size = 11, className }: IconProps & { active?: boolean }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className={className}>
      <polygon points="22 3 2 3 10 12.46 10 19 14 21 14 12.46 22 3" />
    </svg>
  )
}
