...
import { Workflow, FileSearch, Brain, Settings, Home, History, Sparkles, MessageSquare, Radar, ClipboardList, CalendarClock } from 'lucide-react';
...
const INTERACTIVE_AGENTS: SidebarAgent[] = [
  { id: 'scout-9', label: 'Scout-9', icon: 'Search', accentColor: '#3b82f6', status: 'online' },
  { id: 'oracle', label: 'Oracle', icon: 'Sparkles', accentColor: '#06b6d4', status: 'online' },
];

const OPERATIONAL_AGENTS: SidebarAgent[] = [
  { id: 'vigil', label: 'Vigil', icon: 'Radar', accentColor: '#94a3b8', status: 'online' },
  { id: 'braniac', label: 'Braniac', icon: 'Brain', accentColor: '#8b5cf6', status: 'online' },
...
function useBraniacLiveStatus(): AgentLiveStatus {
  const [status, setStatus] = useState<AgentLiveStatus>('online');
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.api?.braniac?.onStatusEvent((event) => {
      if (!mounted) return;
      if (event.status === 'running') {
        setStatus('running');
      } else if (event.status === 'failed') {
...
function getLiveStatus(agentId: string, scout9Status: AgentLiveStatus, vigilStatus: AgentLiveStatus, braniacStatus: AgentLiveStatus, fallback: AgentLiveStatus): AgentLiveStatus {
  if (agentId === 'scout-9') return scout9Status;
  if (agentId === 'vigil') return vigilStatus;
  if (agentId === 'braniac') return braniacStatus;
  return fallback;
}
...
function SubNav({ items, currentPath }: { items: SubNavItem[]; currentPath: string }) {
  const navigate = useNavigate();
  return (
    <div className="ml-6 mr-2 mt-0.5 mb-1 space-y-0.5">
      {items.map(sub => {
        const isSubActive = sub.path === items[0]?.path
...
function getSubNavItems(agentId: string, agentPath: string): SubNavItem[] | null {
  switch (agentId) {
    case 'scout-9':
      return [
        ...
        { path: `${agentPath}/chat`, label: 'Chat', icon: MessageSquare },
        { path: `${agentPath}/settings`, label: 'Settings', icon: Settings },
      ];
    case 'oracle':
      return [
        { path: agentPath, label: 'Home', icon: Home },
        { path: `${agentPath}/chat`, label: 'Chat', icon: MessageSquare },
      ];
    case 'vigil':
      return [
        { path: agentPath, label: 'Mission Control', icon: Radar },
        { path: `${agentPath}/schedule`, label: 'Schedule', icon: CalendarClock },
        { path: `${agentPath}/runs`, label: 'Runs', icon: ClipboardList },
      ];
    case 'braniac':
      return [
        { path: agentPath, label: 'Home', icon: Home },
        { path: `${agentPath}/pipeline`, label: 'Pipeline', icon: Workflow },
        { path: `${agentPath}/history`, label: 'Job History', icon: History },
        { path: `${agentPath}/patterns`, label: 'Patterns', icon: Sparkles },
      ];
    default:
      return null;
  }
...
function AgentSection({ agents, scout9Status, vigilStatus, braniacStatus }: {
  agents: SidebarAgent[];
  scout9Status: AgentLiveStatus;
  vigilStatus: AgentLiveStatus;
  braniacStatus: AgentLiveStatus;
}) {
  ...
  return (
    <>
      {agents.map(agent => {
        ...
        const liveStatus = getLiveStatus(agent.id, scout9Status, vigilStatus, braniacStatus, agent.status as AgentLiveStatus);
        const subNavItems = isActive ? getSubNavItems(agent.id, agentPath) : null;
        ...
        return (
          <div key={agent.id}>
            ...
            >
              ...
              <span
                className={`h-1.5 w-1.5 rounded-full ${getStatusDotClass(liveStatus)}`}
                title={getStatusTitle(liveStatus)}
              />
            </button>

            {subNavItems && (
              <SubNav items={subNavItems} currentPath={location.pathname} />
            )}
          </div>
        );
      })}
...
export default function AgentsLayout() {
  ...
  const braniacStatus = useBraniacLiveStatus();
    ...
    <div className="min-h-screen pb-8 gradient-subtle transition-colors duration-300">
      ...
      <aside className="fixed top-10 left-0 bottom-0 z-50 flex flex-col w-[220px] border-r border-gray-200/30 dark:border-dark-border/30 bg-white/80 dark:bg-dark-surface/80 backdrop-blur-xl">
        ...
        <nav className="flex-1 py-2 overflow-y-auto">
          ...
          <p className="px-4 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted">🗣️ Interactive Agents</p>
          <AgentSection
            agents={INTERACTIVE_AGENTS}
            scout9Status={scout9Status}
            vigilStatus={vigilStatus}
            braniacStatus={braniacStatus}
          />

          <p className="px-4 py-1.5 mt-2 text-[10px] font-semibold uppercase tracking-wider text-muted">⚙️ Operational Agents</p>
          <AgentSection
        ...
        <div className="border-t border-gray-200/30 dark:border-dark-border/30 px-4 py-3">
          <div className="flex items-center gap-1.5">
            <span className={`h-1.5 w-1.5 rounded-full ${getStatusDotClass(
              scout9Status === 'running' || vigilStatus === 'running' || braniacStatus === 'running'
                ? 'running'
                : scout9Status === 'failed' || vigilStatus === 'failed' || braniacStatus === 'failed'
                ? 'failed'
                : 'online'
            )}`} />
            <span className="text-[10px] text-muted font-mono">
              4 online • 3 coming soon
            </span>
          </div>
        </div>
      </aside>
