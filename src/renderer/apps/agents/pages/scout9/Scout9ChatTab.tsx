...
export default function Scout9ChatTab() {
  ...
  useEffect(() => {
    const cleanup = scout9Service.onChatChunkEvent((text) => {
    ...
    <section className="glass-panel h-[calc(100vh-220px)] rounded-2xl p-4 flex flex-col">
      <div className="flex items-center justify-between pb-3 border-b minimal-divider">
        <div className="flex items-center gap-2">
          {agentAvatar && <img src={agentAvatar} alt="Scout-9" className="h-8 w-8 rounded-full object-cover" />}
          <h3 className="text-base font-semibold text-primary">Scout-9 Chat</h3>
          <span className="text-sm text-muted">Ask about candidates, salary feasibility, or staffing strategy</span>
        </div>
