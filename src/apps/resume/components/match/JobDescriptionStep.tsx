...
import { ATSPosition, JdSource, HardConstraints, Seniority, Currency } from '../../types';
...
const SENIORITY_OPTIONS: Seniority[] = ['Junior', 'Mid', 'Senior', 'Lead', 'Architect'];
const CURRENCY_OPTIONS: Currency[] = ['USD', 'MXN', 'COP', 'PEN', 'BOB'];
const COUNTRY_OPTIONS = ['Mexico', 'Colombia', 'Peru', 'Bolivia', 'United States', 'Ireland'];
...
export default function JobDescriptionStep({
  ...
  const [isConstraintsOpen, setIsConstraintsOpen] = useState(
    Boolean(initialConstraints.seniority || initialConstraints.maxRate || initialConstraints.currency || initialConstraints.country)
  );
  ...
  return (
    <div className="space-y-4">
        ...
        {isConstraintsOpen && (
          <div className="px-4 pb-4 border-t border-gray-200/20 dark:border-dark-border/20 pt-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div>
                <label className="block text-xs font-medium text-secondary mb-1.5">Min. Seniority</label>
                <select
                  value={constraints.seniority ?? ''}
                  onChange={(e) => setConstraints((prev) => ({ ...prev, seniority: (e.target.value || undefined) as Seniority | undefined }))}
              ...
              <div>
                <label className="block text-xs font-medium text-secondary mb-1.5">Max Rate ($/hr)</label>
                <input
                  type="number"
                  min={0}
                  value={constraints.maxRate ?? ''}
                  onChange={(e) => setConstraints((prev) => ({ ...prev, maxRate: e.target.value ? Number(e.target.value) : undefined }))}
              ...
              <div>
                <label className="block text-xs font-medium text-secondary mb-1.5">Currency</label>
                <select
                  value={constraints.currency ?? ''}
                  onChange={(e) => setConstraints((prev) => ({ ...prev, currency: (e.target.value || undefined) as Currency | undefined }))}
                  className="glass-input w-full text-sm text-primary"
                >
                  <option value="">Any</option>
      ...
      <button
        ...
        disabled={isDisabled}
        className="w-full py-3 px-6 bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white rounded-xl font-semibold text-sm transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:from-indigo-500 disabled:hover:to-indigo-600"
      >
        Continue
      </button>
    </div>
