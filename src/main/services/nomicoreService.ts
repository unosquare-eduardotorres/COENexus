...
class NomicoreService {
  ...
  async calculate(params: NomicoreCalculateParams): Promise<NomicoreCalculationResult> {
    ...
    try {
      ...
      await this.navigateAndAuth(page, url, phases)
      await this.bootstrapBlazor(page, phases)
      await this.ensureFieldsPopulated(page, params, phases)
      const tablesTimedOut = await this.waitForTableData(page, phases)
      const pageStructure = await this.inspectPageStructure(page)

      if (tablesTimedOut) {
        return this.buildTimeoutResult(page, params, phases, pageStructure)
      }

      return this.extractResults(page, params, phases, pageStructure)
    } finally {
      await ctx.close()
    }
  }

  private async navigateAndAuth(
    page: Page, url: string, phases: NomicoreDiagnostics['phases'],
  ): Promise<void> {
  ...
  private async bootstrapBlazor(
    page: Page, phases: NomicoreDiagnostics['phases'],
  ): Promise<void> {
    try {
  ...
  private async ensureFieldsPopulated(
    page: Page, params: NomicoreCalculateParams, phases: NomicoreDiagnostics['phases'],
  ): Promise<void> {
    ...
    const finderPopulated = inputSnapshot.some(inp => inp.value && parseFloat(inp.value) > 0)

    if (!finderPopulated) {
      log.info('[nomicore] Finder did not auto-populate — filling fields manually', { grossMonthly: params.grossMonthly })
  ...
  private async inspectPageStructure(page: Page) {
    const pageStructure = await page.evaluate(() => {
      ...
      const tableInfo = Array.from(tables).map((table, i) => {
        ...
        return {
          index: i, rows: rows.length, cells: cells.length, thCount: thCells.length,
          headerText: headerText.substring(0, 200),
          sampleHeaders: sampleHeaders.slice(0, 10), sampleCells: sampleCells.slice(0, 10),
          tableClasses: table.className, parentClasses: parent?.className || 'none',
        }
      })
      ...
      const inputs = Array.from(allInputs).map(inp => ({
        type: (inp as HTMLInputElement).type, value: (inp as HTMLInputElement).value,
        name: (inp as HTMLInputElement).name, id: (inp as HTMLInputElement).id,
      }))
      const cards = document.querySelectorAll('.card, div[class*="card"]')
      const bodyText = document.body?.textContent?.substring(0, 500) || ''

      return {
        tableCount: tables.length, tables: tableInfo, headings,
        inputCount: allInputs.length, inputs: inputs.slice(0, 20),
        cardCount: cards.length, bodyTextSnippet: bodyText,
      }
    })

    log.info('[nomicore] Page structure', {
      tableCount: pageStructure.tableCount, headings: pageStructure.headings,
      inputCount: pageStructure.inputCount, cardCount: pageStructure.cardCount,
    })
  ...
  private async buildTimeoutResult(
    page: Page, params: NomicoreCalculateParams,
    phases: NomicoreDiagnostics['phases'], pageStructure: unknown,
  ): Promise<NomicoreCalculationResult> {
    const buffer = await page.screenshot({ fullPage: true })
    const screenshotBase64 = buffer.toString('base64')
    const html = await page.content()
    return {
      params, payroll: {}, cost: {}, profitability: {}, rateCard: {},
      rawHtml: html, screenshotBase64, calculatedAt: new Date().toISOString(),
  ...
  private async extractResults(
    ...
    return {
      params, ...extracted, rawHtml, screenshotBase64,
