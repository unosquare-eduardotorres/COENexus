...
export function registerMatchHandlers(): void {
  ...
  registerIpcHandler(IPC_CHANNELS.MATCH_SEARCH,
    async (event: IpcMainInvokeEvent, request: MatchSearchRequest) => {
      validateSender(event)
      const validated = validatePayload(matchSearchSchema, request, IPC_CHANNELS.MATCH_SEARCH)
  ...
  registerIpcHandler(IPC_CHANNELS.MATCH_SEARCH_SESSION,
    async (event: IpcMainInvokeEvent, params: MatchSearchRequest) => {
      validateSender(event)
      const validated = validatePayload(matchSearchSchema, params, IPC_CHANNELS.MATCH_SEARCH_SESSION)
      const win = getMainWindow()
      const sessionId = await matchEngineService.searchAsync(validated, (evt) => {
        win?.webContents.send(IPC_CHANNELS.MATCH_SEARCH_EVENT, evt)
      })
  ...
  registerIpcHandler(IPC_CHANNELS.MATCH_BENCH_BURN,
    async (event: IpcMainInvokeEvent, params: BenchBurnRequest) => {
      validateSender(event)
      const validated = validatePayload(benchBurnSchema, params, IPC_CHANNELS.MATCH_BENCH_BURN)
      const win = getMainWindow()
  ...
  registerIpcHandler(IPC_CHANNELS.MATCH_BENCH_BURN_RETRY,
    async (event: IpcMainInvokeEvent, params: BenchBurnRequest) => {
      validateSender(event)
      const validated = validatePayload(benchBurnSchema, params, IPC_CHANNELS.MATCH_BENCH_BURN_RETRY)
      const win = getMainWindow()
      const sessionId = await benchBurnService.executeAsync(validated, (evt) => {
        win?.webContents.send(IPC_CHANNELS.MATCH_BENCH_BURN_EVENT, evt)
      })
